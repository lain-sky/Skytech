...
if(!empty($p) && !empty($f)){
	//torrent nev elleneõrzés
	if(!empty($p['tname'])){
		$torrent->name=trim($p['tname']);
	}else $hiba[]="Nem adtad meg a torrent nevét";	

	//kategoria
	if(!empty($p['tcategory']) && is_numeric($p['tcategory'])){
		$torrent->kid=$p['tcategory'];
	}else $hiba[]="Nem választottál kategóriát";
	...
	if($p['tcene']=='yes'){
		...
		if(!empty($p['homepage'])){
			$torrent->honlap=$p['homepage'];
		}else $hiba[]="Nem adtál meg honlapot";

		//megjelenes
		if(!empty($p['year'])){
			$torrent->megjelen=$p['year'];
		}else $hiba[]="Nem adtál meg megjelenést";
	}
	...
	if(!empty($f['tfile']) ){
		...
		if( in_array($f['tfile']['type'],$jo_file_type) !==true){
			...
			d::addText('Hibás torrent fájl',$_FILES);
			d::send(true);
		}
	...
	//torrent megnyitás és ellenõrzés
	if(count($hiba)<1){
	...
	//talán innen kezdõdik a feltöltés
	if(count($hiba)<1){
		...
		if($rogzitett!==false){			
			if($nfoellen===true){
				...
				if(Nfo::add($nfoTemp,$nfoName,$rogzitett)){
					...
					$OLDAL[]=hiba_uzi("Figyelmeztetés! Az hiba az nfo fájl csatolása közben");
				}
			}
