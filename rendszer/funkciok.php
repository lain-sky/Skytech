...
/*
uzenetek megjelenítése
$title az ablakocska tetején megjelenõ szöveg;
$text az üzenet tartalma
$gomb megjelenõ gomb és iránymutatás
$level a kód mélysége a forráskód olvasásánál számít csak!
$width az uzi szélessége
*/
...
// a byte értékét átalakítja szöveges formává
...
//kiszámolja az eltelt idõt és kiirja
function time_to_string($mikor) {
	$stamp = time()-$mikor;
	if ($stamp >= 604800) return (int)($stamp/604800)." hete";
...
function aranyszin($arany) {
	if($arany < 0.5) return("ratio1");
	if($arany < 0.8) return("ratio2");
	if($arany < 1.2) return("ratio3");
	if($arany < 2) return("ratio4");
	return("ratio5");
}
...
// Veletlen torrent pass készítése keszitese
...
// Veletlen megerosito hash keszitese
