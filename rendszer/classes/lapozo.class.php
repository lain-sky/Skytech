...
class lapozo {
	...
	function __construct($cim, $inp = 20, $valt = 'page') {
		...
		if(!empty($_GET[$valt]) && is_numeric($_GET[$valt]))
			$this->page = $_GET[$valt];
		else $this->page = 1;
	}
	...
	function betuvel() {
		...
		if($tomb[0] <= ($this->page - 1))
			...
			$link['elozo'] = '<img border="0" src="kinezet/' . $GLOBALS['USER']['smink'] . '/mini_prev_disabled.png" alt="vissza" />';
		...
		if($tomb[0] >= ($this->page + 1))
			...
			$link['kovetkezo'] = '<img border="0" src="kinezet/' . $GLOBALS['USER']['smink'] . '/mini_next_disabled.png" alt="tovabb" />';
